...
**Dernière mise à jour** : 2026-05-27 (suite — VAGUE 4 démarrée : CMS, cartes/agenda agrégés, dashboard profond, consoles admin Décider/Journal)
**Dernier chantier terminé** : V2.4.13 (`BoutonAdminEditer` systématique sur 9 pages d'espaces individuels : pétitions, mobilisations, cagnottes, moments, sondages, médias, communes, fédérations, groupes-locaux) — sur `main` (tip `1b51896`). Server Component conditionnel, aucun DOM émis si non-admin.
**Dernier chantier terminé (antérieur)** : V2.4.12 (consoles admin Décider + Journal-affiche) — sur `main` (tip `5579c6f`). `/admin/national/decider` (liste salles + formulaire création) + `/admin/national/journal` (liste éditions + formulaire création + boutons publier/brouillon/archiver). 2 actions serveur gardées par `est_admin_general`, slug NFD anti-collision. Nav admin enrichi de 2 entrées.
**Chantiers V2.4 livrés (résumé)** : V2.4.1 CMS minimal (table `contenu_editorial`, helpers + lorem ipsum, `ContenuEditableAdmin` rendu Markdown léger), V2.4.2 helper `estAdminCourant` + `PageEditorialeCMS` Server Component, V2.4.3 cartes agrégées étendues (sondages + groupe_entraide + `chargerPointsHebergement`), V2.4.4 agenda agrégé + catch-all par type, V2.4.5 dashboard membre profond (15 requêtes parallèles, 8 compteurs, activités récentes), V2.4.6 console contenus éditoriaux, V2.4.7 « Mes créations » (14 axes), V2.4.8 `BoutonAdminEditer` Server Component conditionnel, V2.4.9 migration 10 pages éditoriales vers PageEditorialeCMS, V2.4.10 Décider MVP (3 tables salle/reunion/vote + UI publique), V2.4.11 Journal-affiche MVP (table + UI publique), V2.4.12 consoles admin Décider/Journal, V2.4.13 BoutonAdminEditer partout. **3 nouvelles migrations en attente distant** : `20260527130000_contenu_editorial.sql`, `20260527140000_decider.sql`, `20260527150000_journal_affiche.sql` (en plus des 10 V2.x déjà en attente). **13 migrations totales** à appliquer au matin.
