...
**Dernière mise à jour** : 2026-05-27 (suite — VAGUE 4 finitions : file modération globale, planifier réunion Décider, MarkdownLeger réutilisable, ChampImageObjet journal)
**Dernier chantier terminé** : V2.4.17 (téléversement image dans formulaire journal) — sur `main` (tip `e56e905`). Remplace le champ URL nu de la couverture par `ChampImageObjet` (V2.3.4) + `TeleverseurImage` (V2.0.3) ; téléversement direct via Supabase Storage, préfixe `journal-affiche`.
