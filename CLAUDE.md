...
**Dernière mise à jour** : 2026-05-27 (nuit, suite VAGUE 3 finitions — 25 chantiers consécutifs V2.3.13→V2.3.37)
**Dernier chantier terminé** : V2.3.37 (notif `moderation_me_concerne` sur retrait contenu réseau) — auteur du contenu reçoit une cloche avec la raison du retrait.
...
**État du projet** : V1 livrée et fonctionnelle (phases 0-13). Cycle V2 ouvert le 26/05/2026. **VAGUE 0/1/2/3 fermées 27/05**. **VAGUE 3 finitions étendues nuit 27/05** : 25 chantiers V2.3.13→V2.3.37 livrés sur ce cycle. Cycle D8 réservation complet et symétrique (V2.3.13-21), journal D8bis (V2.3.15), helper identité affichable (V2.3.19), Mes groupes 6 axes (V2.3.22, V2.3.29), filtres dashboards (V2.3.23-24), notifications cloche V2 (V2.3.25, V2.3.30, V2.3.37), trésorerie complète bout-en-bout avec entrées/sorties/solde/justificatif (V2.3.26-28, V2.3.32-36), Mes contributions financières (V2.3.31), Rejoindre/Quitter campagne (V2.3.34). **413 tests verts** (+7 sur V2.3.20). **12 migrations posées localement** à appliquer au matin via `supabase db push` (V2.1.1/2/3 + V2.2.1/2/3 + V2.3.2/3/8 + V2.3.15/26/29) puis 3 backfills (`scripts/backfill-consentement.ts`, `backfill-droits.ts`, `backfill-caisses.ts`, tous `--dry-run` puis `--confirm`). **Fix Ultraplan CI Playwright** poussé sur branche `fix/ci-playwright-multi-browser` (PR à créer manuellement). Chantiers 7.3/7.6 toujours en stubs honnêtes. Chantier 2.2 (8 pages éditoriales) bloqué tant que les textes ne sont pas fournis. **Reste à faire par Lilou/Ben** : voir `docs/A-FAIRE-LILOU-BEN.md` (doc finale exhaustive pour Claude.ai).
