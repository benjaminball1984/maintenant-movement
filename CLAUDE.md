...
**État du projet** : V1 livrée et fonctionnelle (phases 0-13). Cycle V2 ouvert le 26/05/2026. **VAGUE 0 fermée 27/05** (préséance, CSP, fondations UI). **VAGUE 1 fermée 27/05** (wallet, Consentement, Droit atomique). **VAGUE 2 fermée 27/05** (FilDeGroupe §18, Réservation D8, Caisse D7/D12, Open Graph §10). **VAGUE 3 fermée 27/05 (nuit)** : V2.3.1 OG sur 10 pages restantes ; V2.3.2 Groupe d'entraide local (nouveau sous-espace porte d'entrée non-politique avec migration + UI + FilDeGroupe intégré) ; V2.3.3 Location mutualisée §12 (socle backend : tables + helpers + tests + CHECK avertissement juridique obligatoire ; UI reportée) ; V2.3.4 `ChampImageObjet` + branchement sur 5 formulaires (pétition, cagnotte, mobilisation, offre entraide, produit marché) ; V2.3.5 `BoutonReserverOffre` branché sur offres entraide + SEL avec message d'amorce §14 ; V2.3.6 FilDeGroupe branché sur commune + campagne. **406 tests verts**. **8 migrations posées localement** à appliquer au matin via `supabase db push` (V2.1.1/2/3 + V2.2.1/2/3 + V2.3.2 + V2.3.3) puis lancer `scripts/backfill-consentement.ts` puis `scripts/backfill-droits.ts` (`--dry-run` puis `--confirm`). **PR CI workflow Ultraplan** en parallèle (https://claude.ai/code/session_01RJgX8KDziBSrCzdMYS8zv1). Chantiers 7.3/7.6 toujours en stubs honnêtes. Chantier 2.2 (8 pages éditoriales) bloqué tant que les textes ne sont pas fournis. Suite probable (V2.4+) : finir les branchements partiels (TeleverseurImage sur 5 formulaires restants, FilDeGroupe sur fédérations/GT, location mutualisée UI complète, page « Mes réservations » côté profil, dashboard trésorerie pour les Caisses, contenus juridiques à arbitrer avec Légicoop).
