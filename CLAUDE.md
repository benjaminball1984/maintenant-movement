...
**État du projet** : V1 livrée et fonctionnelle (phases 0-13). Cycle V2 ouvert le 26/05/2026. **VAGUE 0/1/2/3 fermées 27/05** (préséance, CSP, fondations UI / wallet+consentement+droit / FilDeGroupe+Réservation+Caisse+OG / branchements et symétries). **VAGUE 3 finitions livrées dans la nuit du 27/05** : V2.3.7 ChampImageObjet sur 5 formulaires de plus (édition pétition + création minimarché/boutique/campagne/sondage) ; V2.3.8 fix critique helper SQL `est_membre_espace` ; V2.3.9 page « Mes réservations » côté profil ; V2.3.10 dashboard trésorerie lecture seule ; V2.3.11 annulation réservation côté demandeur ; V2.3.12 envoi automatique du message d'amorce dans `message_reseau` ; **V2.3.13 dashboard propriétaire d'offre (`/profil/demandes-reservations` + Server Actions accepter/refuser/marquer réalisée)** ; **V2.3.14 confirmation côté demandeur + correction du branchement Annuler V2.3.11** ; **V2.3.15 journal des transitions D8 (doctrine D8bis, `reservation_journal` + UI historique)** ; **V2.3.16 bouton « Signaler un litige » côté demandeur**. Cycle D8 désormais navigable de bout en bout. **406 tests verts**. **10 migrations posées localement** à appliquer au matin via `supabase db push` (V2.1.1/2/3 + V2.2.1/2/3 + V2.3.2/3/8 + V2.3.15) puis lancer `scripts/backfill-consentement.ts` puis `scripts/backfill-droits.ts` (`--dry-run` puis `--confirm`). **PR CI workflow Ultraplan** en parallèle (https://claude.ai/code/session_01RJgX8KDziBSrCzdMYS8zv1). Chantiers 7.3/7.6 toujours en stubs honnêtes. Chantier 2.2 (8 pages éditoriales) bloqué tant que les textes ne sont pas fournis. Suite probable (V2.4+) : modération admin des réservations en litige, helper `nomAffichageRespectantVisibilite`, notifications V2 quand la table sera là, UI complète location mutualisée (juridique Légicoop), branchement Caisses aux flux d'entrée V1.
