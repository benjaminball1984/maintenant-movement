...
**État du projet** : V1 livrée et fonctionnelle (phases 0-13 : squelette technique complet, 33 chantiers livrés, 4.3 à 11.3 puis 12.1-12.6 + 7.5 réseau social + 13.x intégration plateforme de données). Chantiers 7.3/7.6 en stubs fonctionnels honnêtes (à enrichir post-MVP). Chantier 2.2 (8 pages éditoriales) bloqué tant que Lilou/Ben ne fournit pas les textes ; les pages affichent une bannière neutre en prod. Lighthouse mobile et audit WCAG complet à faire après un déploiement de staging Cloudflare Pages. **Cycle V2 ouvert le 26/05/2026** : greffe additive selon le CDC V2 (`docs/cdc-v2/`). **VAGUE 0 fermée le 27/05/2026 (nuit)** : V2.0.1 préséance + dépôt CDC + coquilles ; V2.0.2 CSP nominative + README `app/auth/` ; V2.0.3 fondations UI (variant `primary`, bouton bascule thème branché BDD, composant `TeleverseurImage` + adapter Storage, bibliothèque d'images par défaut). **VAGUE 1 fermée le 27/05/2026 (nuit)** : V2.1.1 retrait wallet intégré (page supprimée, méthode `envoyerTransaction` DEPRECATED, garde-fou `t99cp_hash_consomme`) ; V2.1.2 entité `Consentement` granulaire + révocable (greffe à côté de `signature_petition`, backfill `--dry-run` prêt) ; V2.1.3 table `droit` atomique + 5 presets V2 + 6 presets V1 + projection pure depuis `droit_admin` + backfill `--dry-run` prêt. 337 tests verts. **3 migrations posées localement à appliquer au matin** (`supabase db push`) : `20260527000000_t99cp_hash_consomme.sql`, `20260527010000_consentement.sql`, `20260527020000_droit.sql`. Puis lancer `scripts/backfill-consentement.ts` puis `scripts/backfill-droits.ts` (`--dry-run` puis `--confirm`). VAGUE 2 (FilDeGroupe, Réservation réutilisable, Caisse + reversements, Open Graph serveur) à venir, **demande autorisation explicite avant exécution** (touche nouveaux concepts et pages publiques).
