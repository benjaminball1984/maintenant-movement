...
**Dernier chantier terminé** : 13 (intégration) — Les trois chantiers parallèles de la phase 13 ont été finis et fusionnés sur `feature/phase-13-integration` (commit b141b6d, depuis la base saine 13.1 d01562b ; `main` non touchée). 13.1 : fix inscription/RLS + édition des pétitions par l'équipe (formulaire, dates lancement/échéance migration 035, pages `/admin/petitions`). 13.2 : console nationale (super admin, gestion des droits, journal d'audit) + modération active complète (retrait sur tous les domaines, dont `retirerServiceSel` et `reafficherOrganisation`). 13.3 : plateforme de données (table `commune_reference`, correspondance CP/INSEE, scripts d'import validés en dry-run). 279 tests verts, lint + typecheck verts (voir `docs/manifests/phase-13-integration.md`).
